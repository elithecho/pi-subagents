# @elithecho/pi-subagents

A [pi](https://pi.dev) extension that brings **Claude Code-style autonomous sub-agents** to pi. Spawn specialized agents that run in isolated sessions — each with its own tools, system prompt, model, and thinking level. Run them in foreground or background, steer them mid-run, resume completed sessions, and define your own custom agent types.

> **Status:** Early release.

<img width="600" alt="pi-subagents screenshot" src="https://github.com/elithecho/pi-subagents/raw/master/media/screenshot.png" />
...
- **Panic stop** — open a running agent from `/agents` and press `x` in the conversation viewer to stop it; stop-all is also available in `/agents → Settings`
...
| `context_limit` | unlimited | Max lifetime tokens (input + output + cacheWrite) before the agent is forced to wrap up. When crossed, a loud steering message tells the agent to stop, end its turn, and report back with whatever it has. `0` or omit for unlimited |
...
Frontmatter is authoritative. If an agent file sets `model`, `thinking`, `max_turns`, `context_limit`, `inherit_context`, `run_in_background`, `isolated`, or `isolation`, those values are locked for that agent. `Agent` tool parameters only fill fields the agent config leaves unspecified.
...
To stop background work immediately, open a running agent from `/agents` and press `x` in the conversation viewer to stop that agent, or use `/agents → Settings → Stop all running/queued agents`.
