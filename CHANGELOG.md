...
### Changed
- **Default max turns is now 60** when no per-agent, tool-call, global, or project setting is provided. Users can still set `defaultMaxTurns: 0` in subagent settings, enter `0` in `/agents` → Settings, or use `max_turns: 0` in agent frontmatter for unlimited turns.
